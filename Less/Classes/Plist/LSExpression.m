...
- (instancetype)initWithUTF8String:(const char *)str {
    ...
    switch (*p) {
        ...
        case '>':
            p++;
            if (*p == '@') {
                _tag = @">@"; // input value for `LSForm'
                p++;
            } else {
                _tag = @">"; // input text for `LSForm'
            }
            break;
        default:
            return nil;
    ...
    if (![[LSVariableMapper allTags] containsObject:_tag]) {
...
- (void)dealloc
{
    if (_flags.duplexBinding) {
        [[NSNotificationCenter defaultCenter] removeObserver:self name:LSViewWillRemoveFromSuperviewNotification object:nil];
    }
}
...
{
    if (_tag == nil) {
        return [LSValueParser valueWithString:_rvalue];
    }
    id (^mapper)(id data, id target, int index) = [LSVariableMapper mapperForTag:_tag];
    if (mapper == nil) {
        return nil;
    }
...
- (id)valueByOperatingValue:(id)value
{
    ...
    if (_flags.test) {
        ...
        return [LSValueParser valueWithString:value];
    }
    return value;
}
...
- (void)mapData:(id)data toOwner:(id)owner forKeyPath:(NSString *)ownerKeyPath
{
    ...
    switch (flag) {
        case '!': { // Event
            ...
            id (^mapper)(id data, id target, int index) = [LSVariableMapper mapperForTag:_tag]; // default mapper to active controller
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    if ([keyPath isEqualToString:_variable]) {
        ...
        if (self.parent != nil) {
            LSMutableExpression *parent = (id) self.parent;
