...
{
    if (_lsCollectionViewFlags.deallocing) {
...
- (void)dealloc {
    ...
    _lsCollectionViewFlags.deallocing = 1;
}

- (void)setItem:(NSDictionary *)item {
    _itemMapper = [LSRowMapper mapperWithDictionary:item];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    LSRowMapper *item = [self itemAtIndexPath:indexPath];
...
- (void)setAutoResize:(BOOL)autoResize {
    _lsCollectionViewFlags.autoResize = autoResize ? 1 : 0;
}

- (BOOL)isAutoResize {
    return (_lsCollectionViewFlags.autoResize == 1);
}

#pragma mark - Helper

- (LSRowMapper *)itemAtIndexPath:(NSIndexPath *)indexPath
{
    LSRowMapper *item = _itemMapper;
    if (item == nil) {
        if (self.sections != nil) {
            LSSectionMapper *section = [self.sections objectAtIndex:indexPath.item];
            if (section != nil) {
        ...
        if (item == nil) {
            [NSException raise:@"LSTableViewError" format:@"Missing row spec!"];
        }
    }
    return item;
}
...
- (id)dataAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    } else if ([_data isKindOfClass:[LSSection class]]) {
        return [(LSSection *)_data recordAtIndexPath:indexPath];
    }
