...
NSString *const LSPopdownCoverWillShowNotification = @"LSPopdownCoverWillShow";
NSString *const LSPopdownCoverDidShowNotification = @"LSPopdownCoverDidShow";
NSString *const LSPopdownCoverWillHideNotification = @"LSPopdownCoverWillHide";
NSString *const LSPopdownCoverDidHideNotification = @"LSPopdownCoverDidHide";
NSString *const LSPopdownCoverViewKey = @"LSPopdownCoverView";

@implementation UIButton (LSForm)
...
DEF_UNDEFINED_LSOPERTY2(NSString *, action, setAction)
DEF_UNDEFINED_BOOL_LSOPERTY(isRequired, setRequired, NO)
DEF_UNDEFINED_LSOPERTY2(NSString *, requiredTips, setRequiredTips)
DEF_UNDEFINED_LSOPERTY2(NSDictionary *, actionProperties, setActionProperties)

DEF_UNDEFINED_LSOPERTY2(UILabel *, placeholderLabel, setPlaceholderLabel)
...
- (void)onHrefClick:(id)sender {
    LSViewClickHref(sender, [sender href]);
}
...
- (void)showControllerByType:(NSString *)type {
    ...
    if ([controller conformsToProtocol:@protocol(LSTextInput)]) {
        [(id<LSTextInput>)controller setInputDelegate:self];
    }
...
- (void)textInput:(id<LSTextInput>)textInput didInputText:(NSString *)text value:(id)value {
    ...
    while (form) {
        if ([form isKindOfClass:[LSForm class]]) {
            break;
        }
        form = [form superview];
    }
...
- (void)switchPopdownView:(UIView *)view toView:(UIView *)newView fromWindow:(UIWindow *)window {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:LSPopdownCoverWillHideNotification object:self userInfo:@{LSPopdownCoverViewKey:view}];
    if (newView != nil) {
...
//            [view setAlpha:0];
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:LSPopdownCoverDidHideNotification object:self userInfo:@{LSPopdownCoverViewKey:view}];
...
- (void)showPopdownView:(UIView *)view toWindow:(UIWindow *)window showsCover:(BOOL)showsCover {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:LSPopdownCoverWillShowNotification object:self userInfo:@{LSPopdownCoverViewKey:view}];
    CGRect fromRect = [self popdownButtonRect:view];
    [view setFrame:fromRect];
//    [view setAlpha:0];
...
//        [view setAlpha:1];
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:LSPopdownCoverDidShowNotification object:self userInfo:@{LSPopdownCoverViewKey:view}];
    }];
}
